...
| **BigQuery `access_grants` ledger** | Append-only record: REQUEST / DENY / EXPIRE_FLAG (no GRANT rows; the broker writes none, ADR-006) | Denials matter as much as grants. A denial with a reason is the evidence that the control fired. |
...
| **GCS DATA_READ audit config** (`terraform/logging.tf`) | Enables Cloud Storage DATA_READ audit logs project-wide | Closes the in-window observation gap: PAM logs that a grant was issued, but without DATA_READ the reads an underwriter makes *under* that grant are recorded nowhere. Now the in-window reads land in Cloud Logging and the platform export, so what happens during the 30 minutes is observed, not just that the grant existed. |
| **`reconcile`** (Pub/Sub, `*/15`) | Reconstructs grants from the PAM `CreateGrant` audit events in `bankvault_platform_logs` (the broker writes no GRANT row, ADR-006), finds any past `window_end` with no close-out row, cross-checks live PAM grant state, writes EXPIRE_FLAG and alerts | It **detects** an overrun. It does not contain one. PAM owns expiry, and a second enforcement path would be a second thing that can be wrong. (ADR-005.) |
...
By the write path and the SA's permissions, not by a BigQuery feature. And be precise if they push: BigQuery IAM has no insert-only permission — `bigquery.tables.updateData` covers insert, update, and delete alike, so append-only *cannot* be expressed as an IAM boundary even if you wanted it to be. `deletion_protection` blocks dropping the table, not rewriting rows. The honest statement is "append-only by convention and least privilege, with an independent platform-log export as the tamper-evidence layer." Genuine immutability needs a WORM-backed store, and that part *is* in the build: `terraform/logging.tf` writes the platform-log export to a GCS bucket with a locked retention policy, in parallel with the queryable BigQuery copy. I'd be precise about the split — the BigQuery ledger stays append-only-by-convention; the WORM bucket is where immutability is actually delivered, and only for the log stream it holds. If you also needed the queryable ledger itself locked, that's scheduled table snapshots, and that one I'd still flag as a next step.

**Q: What happens when the identity provider is down?**
Access is denied, and be precise about *where* the denial is enforced: it's the ACM reauth layer, not the broker. If the IdP is unreachable, the underwriter can't complete the hourly reauthentication, so there's no fresh session and the PAM request path is closed — that holds whether or not they call the broker. The broker's own IdP-denial is the same dependency showing up on the skippable pre-flight path, not a second gate. That's deliberate and it has a real cost. A loan decision with an SLA doesn't stop having one because Okta is down. I took the trade because an identity control that keeps granting when it can't verify who's asking has a bypass, and the bypass opens under exactly the conditions an attacker wants: the IdP degraded, checks failing open, nobody watching. If a specific SLA can't tolerate it, the answer is a documented, alerted break-glass path, not a control that quietly fails open.
...
**Q: What breaks the reconcile sweep?**
Three known failure modes, all documented in `docs/architecture.md` under "Known failure modes." None is a privilege path — reconcile is detect-only, so the worst case is duplicate, delayed, or false evidence, never a missed revocation. (1) The sweep dedups by querying the flag rows a prior run wrote, but those are BigQuery *streaming* inserts, and Pub/Sub redelivery (at-least-once, `RETRY_POLICY_RETRY`) can re-run the sweep before the streaming buffer is queryable — so a redelivery after a partial failure can write a duplicate `EXPIRE_FLAG` / `BYPASS_FLAG` row. (2) The same buffer lag can produce a false-positive `BYPASS_FLAG`: a legitimate grant created seconds before a tick, whose broker `REQUEST` row hasn't settled, looks like a bypass — which is why the bypass match is stated as a heuristic to investigate, not proof. (3) The overrun loop has no per-row error isolation, so one unreadable PAM grant aborts the whole sweep, which then self-heals on the next cron but compounds the duplicate-row race. I know the fixes — deterministic `insertId`, a grace window, per-row `try/except` — and I chose to document the bounds rather than build robustness the demo doesn't exercise. The point I'd make is that I found these, not that they don't exist.
