...
| Logical access — audit trail | Explicit | Append-only `access_grants` ledger plus an independent platform-log export. A grant issued with no broker pre-flight is itself flagged (`BYPASS_FLAG`) by the reconcile sweep, so a bypass is recorded rather than invisible. `terraform/bigquery.tf`, `terraform/logging.tf`, `functions/reconcile/main.py`. *Append-only is enforced by the write path and least privilege, not by BigQuery — BQ IAM cannot express insert-only. The independent platform-log export is the tamper-evidence layer, and its WORM immutability copy is now defined in `terraform/logging.tf` (a second sink to a retention-locked GCS bucket); the BigQuery ledger itself stays append-only-by-convention.* |
