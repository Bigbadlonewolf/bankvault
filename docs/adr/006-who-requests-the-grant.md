...
**A raw REST call outside the SDK is the one path a narrowed binding may not reach — and it's why the unnarrowed binding is the default.** Resolved against current docs: `scopedAccessSettings` targets clients by OAuth `clientId`, and "Google Cloud SDK" is one of them, so a binding scoped to the SDK *does* gate `gcloud pam grants create` — the realistic underwriter path is covered. What a Console/SDK-scoped binding may not gate is a request built as a raw REST call bearing a user token from some other client. That residual is exactly why this project uses the unnarrowed binding: it covers the group's whole Google Cloud session regardless of client, at the cost of the broad blast radius above. The narrowed form would trade blast radius for a coverage gap on the raw-REST path; the unnarrowed form is the safer default and is what the enforcement claim rests on.
