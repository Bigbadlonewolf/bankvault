...
- Reconcile has three **known, documented** failure modes (`docs/architecture.md` → "Known failure modes"): duplicate flag rows on Pub/Sub redelivery (streaming-insert dedup race), false-positive `BYPASS_FLAG` from the same buffer lag, and one unreadable PAM grant aborting the whole sweep (no per-row `try/except`). These are deliberately documented rather than fixed — the demo doesn't need the robustness. If you build a fix (deterministic `insertId`, a grace window, per-row error isolation), it's a real improvement, but update the docs so the "known limitation" framing goes with it. Do not treat them as undiscovered bugs.
